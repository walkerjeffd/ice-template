...
ICE provides a generalized approach for creating web-based interactive data visualizations of environmental datasets and model outputs. ICE was designed primarily for exploring geospatial patterns, and was not intended for exploring temporal (timeseries) data.
...
WARNING: use and modification of this template requires experience in modern web application development, JavaScript programming, and the use of the various third-party libraries (specifically vue.js, d3.js, crossfilter.js, leaflet.js, among others).
...
Complete the following steps to run the demo application included in this repo, which provides access to two example datasets.
...
These files are organized by theme within the `data/` folder. The files for each theme must be stored within a subfolder named after the theme id (e.g., `data/northeast/` contains the files for the `northeast` theme).
...
```json
{
  ...
  "description": "A dummy variable from 0 to 1", // additional description
...
For each theme, create a new vue component in the `src/components/themes/` folder. This component is used to display the details of a selected feature. For the demo datasets, each component (`src/components/themes/Northeast.vue` and `src/components/themes/Northwest.vue`) simply provides a pair of tables listing the values of each variable. However, these components can be greatly enhanced to include other data and charts. For more complex examples, see the `ice-lmg` repo.
...
Lastly, update the text content in the various dialog boxes and the application title in the `App.vue` file.
